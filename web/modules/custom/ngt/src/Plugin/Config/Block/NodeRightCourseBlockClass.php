...
use Drupal\ngt\Plugin\Block\NodeRightCourseBlock;
...
class NodeRightCourseBlockClass {
    ...
    /**
     * @param \Drupal\ngt\Plugin\Block\NodeRightCourseBlock $instance
     * @param $config
     */
    ...
    public function preparerate($nodes){
        ...
        foreach ($nodes as $node) {
            ...
            $course = [
                ...
                'autor' => \Drupal::service('ngt.course_main')->load_author($node->get('field_autor_principal')->getValue()),
                'expertos' => \Drupal::service('ngt.course_main')->load_author($node->get('field_expertos')->getValue()),
                'foto_portada' => [
                    'uri' => \Drupal::service('ngt.course_main')->load_image($node->get('field_foto_portada')->getValue()[0]['target_id']),
                    'uri_360x196' => \Drupal::service('ngt.course_main')->load_image($node->get('field_foto_portada')->getValue()[0]['target_id'],'360x196'),
                    'uri_604x476' => \Drupal::service('ngt.course_main')->load_image($node->get('field_foto_portada')->getValue()[0]['target_id'],'604x476'),
                ...
                'video' => \Drupal::service('ngt.course_main')->load_url_file($node->get('field_video')->getValue()[0]['target_id']),
            ];
