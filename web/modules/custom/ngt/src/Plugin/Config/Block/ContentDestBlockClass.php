...
use Drupal\ngt\Plugin\Block\ContentDestBlock;
...
class ContentDestBlockClass {
    ...
    /**
     * @param \Drupal\ngt\Plugin\Block\ContentDestBlock $instance
     * @param $config
     */
        ...
        foreach ($nodes as $node) {
            $content = [
                ...
                'foto_portada' => [
                    ...
                    'uri_313x156' => \Drupal::service('ngt.course_main')->load_image($node->get('field_foto_portada')->getValue()[0]['target_id'],'313x156'),
                    'uri_604x476' => \Drupal::service('ngt.course_main')->load_image($node->get('field_foto_portada')->getValue()[0]['target_id'],'604x476'),
                    'uri_374x226' => \Drupal::service('ngt.course_main')->load_image($node->get('field_foto_portada')->getValue()[0]['target_id'],'374x226'),
